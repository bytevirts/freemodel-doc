...
This guide takes you from a new account to a working FreeModel request.

## Step 1: Get an API key

1. [Create a FreeModel account](https://freemodel.app/sign-up).
...
| Image | `gpt-image-2` | from \$0.021 per image |
...
```bash
curl "https://api.freemodel.app/v1/videos/$VIDEO_ID?model=gemini-omni-t2v" \
  -H "Authorization: Bearer $FREEMODEL_API_KEY"
```
...
```bash
curl -L "https://api.freemodel.app/v1/videos/$VIDEO_ID/content?model=gemini-omni-t2v" \
  -H "Authorization: Bearer $FREEMODEL_API_KEY" \
  --output video.mp4
```
