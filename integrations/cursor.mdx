---
title: "Cursor"
description: "Use FreeModel text models through Cursor's OpenAI API key settings."
---

Cursor includes an OpenAI API key section with an **Override OpenAI Base URL** option. Point that option to FreeModel and add the exact model ID you want to use.
...
<Steps>
  ...
  <Step title="Add the API key">
    Enable **OpenAI API Key**, enter your FreeModel key, and keep it in Cursor's
    credential store.
  </Step>
  <Step title="Override the base URL">
    Enable **Override OpenAI Base URL** and enter
    `https://api.freemodel.app/v1`.
  </Step>
...
<AccordionGroup>
  ...
  <Accordion title="Cursor keeps calling OpenAI">
    ...
    `https://api.freemodel.app/v1` value.
  </Accordion>
