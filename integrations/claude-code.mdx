---
title: "Claude Code"
description: "Connect Claude Code to FreeModel through the Anthropic Messages-compatible gateway."
---

Claude Code can connect to an LLM gateway through `ANTHROPIC_BASE_URL` and a bearer token. FreeModel exposes `POST /v1/messages` and model discovery for this workflow.

<Note>
  Anthropic documents gateway connectivity but does not officially support
  routing Claude Code to non-Claude models. FreeModel currently converts Claude
...
```bash
export ANTHROPIC_BASE_URL="https://api.freemodel.app"
export ANTHROPIC_AUTH_TOKEN="$FREEMODEL_API_KEY"
export CLAUDE_CODE_ENABLE_GATEWAY_MODEL_DISCOVERY=1
```

Use `ANTHROPIC_AUTH_TOKEN`, not `ANTHROPIC_API_KEY`. FreeModel expects `Authorization: Bearer ...`; Claude Code sends that header for `ANTHROPIC_AUTH_TOKEN`.
...
Run `/status` inside Claude Code. Confirm that the Anthropic base URL is `https://api.freemodel.app` and the credential source is `ANTHROPIC_AUTH_TOKEN`.
...
{
  "env": {
    "ANTHROPIC_BASE_URL": "https://api.freemodel.app",
...
<AccordionGroup>
  ...
  <Accordion title="Requests return 401">
    ...
    `ANTHROPIC_API_KEY` is sent through `x-api-key`, which FreeModel does not use.
  </Accordion>
</AccordionGroup>
