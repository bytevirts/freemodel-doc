---
title: "Cline"
description: "Configure Cline's OpenAI Compatible provider for FreeModel."
---
...
<Steps>
  ...
  <Step title="Enter gateway settings">
    Set **Base URL** to `https://api.freemodel.app/v1`, enter your FreeModel API
    key, and set **Model ID** to an exact text model ID.
  </Step>
  <Step title="Set displayed pricing">
    Enter the model's input and output price from its FreeModel model page so
...
<AccordionGroup>
  ...
  <Accordion title="Cline's cost display is wrong">
    Cline does not fetch FreeModel pricing automatically. Update its model
