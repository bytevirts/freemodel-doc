---
title: "Gemini CLI"
description: "Point Google Gemini CLI at FreeModel's Gemini native compatibility endpoint."
---

Gemini CLI uses Google's native `generateContent` protocol. FreeModel exposes that protocol at `/v1beta/models/{model}:generateContent` and accepts the current GPT text model IDs.
...
```bash
export GEMINI_API_KEY="$FREEMODEL_API_KEY"
export GOOGLE_GEMINI_BASE_URL="https://api.freemodel.app"
```
...
<Note>
  Gemini CLI's native request format was verified against FreeModel with
