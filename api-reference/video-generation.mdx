...
All FreeModel video routes use the same asynchronous workflow. Choose a model from the **Video Series** sidebar, submit a request, save the returned `id`, and poll until the task completes.
...
<ParamField body="webhook" type="string">
  Optional public callback URL. FreeModel sends the final task object when the
...
curl -L "https://api.freemodel.app/v1/videos/task_01HXYZ/content?model=video-ds-2.0-fast" \
  -H "Authorization: Bearer $FREEMODEL_API_KEY" \
  --output video.mp4
```
