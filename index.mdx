---
title: "FreeModel API"
sidebarTitle: "FreeModel API overview"
description: "Use one API key for GPT text, image generation, and asynchronous video generation."
---

FreeModel is a prepaid AI gateway with OpenAI-compatible, Anthropic-compatible, and Gemini-compatible text protocols plus image and video generation APIs.

Change your base URL to `https://api.freemodel.app/v1`, keep your API key on the server, and select a model ID from the live catalog.

## Quick start

<Steps>
  <Step title="Create an API key">
    [Create an account](https://freemodel.app/sign-up), add balance, then create
...
<CardGroup cols={2}>
  ...
  <Card title="Image Series" icon="image" href="/api-manual/image/gpt-image-1">
...
FreeModel uses prepaid USD balance with no subscription. Text and GPT Image 1 routes are token-billed. GPT Image 2 and video routes use fixed or duration-based request prices.

<Note>
  The [live models page](https://freemodel.app/models) is the source of truth for
