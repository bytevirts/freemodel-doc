...
`gemini-omni-extend` continues motion from supplied public source assets. The FreeModel compatibility request still uses the `image_urls` field for one to five source URLs.
...
<ParamField body="seconds" type="string" required>
  Set to `"10"`. FreeModel validates this fixed task duration.
</ParamField>
...
<ParamField body="image_urls" type="string[]" required>
  ...
  FreeModel compatibility API even when the source represents extension input.
</ParamField>
...
<Note>
  `seconds` is required by the public FreeModel request even though the
